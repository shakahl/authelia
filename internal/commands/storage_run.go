...
func (ctx *CmdCtx) StorageUserTOTPGenerateRunE(cmd *cobra.Command, args []string) (err error) {
	...
	if c, err = totpProvider.GenerateCustom(args[0], ctx.config.TOTP.DefaultAlgorithm, secret, ctx.config.TOTP.DefaultDigits, ctx.config.TOTP.DefaultPeriod, ctx.config.TOTP.SecretSize); err != nil {
		return err
	}
